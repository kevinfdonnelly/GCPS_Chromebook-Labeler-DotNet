...
public sealed class DymoPrinterService
{
    ...
    public Task<bool> PrintStudentLabelAsync(string name,string studentId,string chromebookId,string countyId)=>PrintAsync("student_30336.dymo",new Dictionary<string,string>{{"NAME",name},{"STUDENT_ID_TEXT",studentId},{"CHROMEBOOK_ID",chromebookId},{"STUDENT_BARCODE",studentId},{"COUNTY_BARCODE",countyId},{"COUNTY_ID_TEXT",countyId}},RightRoll);
    public Task<bool> PrintCountyLabelAsync(string countyId)=>PrintAsync("county_30332.dymo",new Dictionary<string,string>{{"COUNTY_BARCODE",countyId},{"COUNTY_ID_TEXT",countyId}},LeftRoll);
